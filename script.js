...
//var generatePasswordButton = document.getElementById("generate");
 ...
 const generatePassword = ( hasOnlyUpper, hasOnlyLower, hasOnlysSpecial ,hasOnlyNumeric) => {
...
const availableCharacters = [
  ...(hasOnlyUpper ? onlyUpper : []),
  ...(hasOnlyLower ? onlyLower : []),
  ...(hasOnlysSpecial ? onlySpecial: []),
  ...(hasOnlyNumeric ? onlyNumeric : []),
];

for (let i = 0; i < length; i++){
  const randomIndex = Math.floor(Math.random()* availableCharacters.length)
  password += availableCharacters.length[randomIndex];
